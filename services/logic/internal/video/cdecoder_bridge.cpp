...
#include "cdecoder_h264_depacketizer.hpp"
#include "cdecoder_rtp.hpp"

namespace {

// One RTPGWDecoderSession = one video stream's full RTP -> raw frame
// pipeline, identical in shape to what mf_cpp's MF class used to run for
// itself (AccessUnitAssembler -> H264Depacketizer -> FFmpegDecoder), now
// owned by RTPGW instead. See rtpgw_design_v3.md section 37.
...
private:
    mf::AccessUnitAssembler assembler_;
    mf::H264Depacketizer depacketizer_;
    mf::FFmpegDecoder decoder_;
    int width_, height_;
};
